...
from database_metrics import (
    ...
    get_avg_price,
    get_avg_year,
    get_most_expensive_shoe,
    get_user_post_count,
    get_user_follower_count,
    get_for_sale_posts,
    get_following_count,
    get_favorite_shoe_brand,
)
import uvicorn
...
def add_post(payload: AddPostPayload):
    ...
    print(shoe_id)
    if shoe_id is None:
        shoe_id = add_shoe_to_database(
            payload.brand, payload.model, payload.year, payload.color
        )
    updated = add_post_to_database(
        payload.caption,
        payload.picture_url,
        payload.is_selling,
        payload.price,
        payload.selling_link,
        payload.date,
        userData[0]["user_id"],
        shoe_id,
    )
